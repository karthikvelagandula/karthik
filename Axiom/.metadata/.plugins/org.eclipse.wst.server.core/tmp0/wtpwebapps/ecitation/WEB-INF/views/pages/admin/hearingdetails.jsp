		...
		<div>
			<div class="portlet light portlet-fit bordered">
				...
				<div class="portlet-title">
					...
					<a href="<c:url value='./add/${citationId}' />"
						...
						<table
							...
							<thead>
								<tr>
									...
									<th>Disposition/<br>User ID
									</th>
									<th>Suspend/<br>Suspend Until
									</th>
									<th>Case # /<br>Plea
									</th>
									<th>Reduction/<br>Total Due
									</th>
								</tr>
							</thead>
							<tbody>
								<c:choose>
									<c:when test="${empty hearingDetailsList}">
										<tr>
											<td colspan="11" align="center"><b>No results found</b></td>
										</tr>
									</c:when>
									<c:otherwise>
										<c:forEach items="${hearingDetailsList}" var="hearing">
											<tr class="odd gradeX">
												<td><label
													class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
												...
												<td><b><a style="color: #32c5d2"
														href="<c:url value='editHearing/${hearing.id}' />"> <c:out
																value="${hearing.citation.citationId}" />
													</a></b></td>
												<td><c:out value="${hearing.hearingOfficer}" /></td>
												<td><c:out value="${hearing.formattedHearingDate}" />/<c:out
														value="${hearing.hearingTime}" /></td>
												<td><c:out value="${hearing.formattedDispDate}" />/<c:out
														value="${hearing.dispositionTime}" /></td>
												<td><c:out value="${hearing.disposition}" />/<c:out
														value="${hearing.userId}" /></td>
												<td><c:out value="${hearing.suspend}" />/<c:out
														value="${hearing.formattedSuspendDate}" /></td>
												<td><c:out value="${hearing.caseNumber}" />/<c:out
														value="${hearing.plea}" /></td>
												<td>$<c:out value="${hearing.reduction}" />/$<c:out
														value="${hearing.citation.totalDue}" /></td>
											</tr>
										</c:forEach>
									</c:otherwise>
								</c:choose>
